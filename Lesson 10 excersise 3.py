...
def get_float_from_user(msg):
    while(True):
        try:
            user_input = float(input(msg).replace(",","."))
            return user_input
        except ValueError:
...
def get_user_data():
  ...
  flag = True
  while (flag):
      name = get_string_from_user("Enter your name: ")
      if(len(name)<2):
  ...
  flag = True
  while (flag):
      W = get_float_from_user("Enter your weight in kilograms: ")
      if(W<5):
  ...
  flag = True
  while(flag):
      H_cm = get_float_from_user("Enter your height in centimeters: ")
      if(H_cm<50):
...
print(f"{user_data['Name']} weights {user_data['weight']}kg and is {user_data['height']} meters tall.")
